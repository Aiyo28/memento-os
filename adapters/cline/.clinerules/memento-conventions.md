...
| `[E]` | Error | A mistake with root cause — defaults to settled |
| `[S]` | Seed | A deferred idea — activates when its condition is met, not when time passes |

Optional expiry (for time-bound conclusions only):
```
[D] conclusion — invalidates if trigger — expires: YYYY-MM-DD [priority] [YYYY-MM-DD]
```
Do NOT add expiry to foundational architecture decisions.

## Priority Matrix

Priority is derived from **confidence × impact**:
...
- AI proposes confidence (high / medium / low)
- Ask user: "High or low impact?"
- `[E]` errors default to **settled**; once resolved → **noise**
- **noise** priority: ask "store anyway or discard?" — default: discard

## Kobe Cap: 24 Artifacts

Maximum 24 artifacts per project in the Active Reasoning Artifacts table.

Eviction order when over cap: noise → settled → volatile → critical (never auto-evict).
Archive evicted artifacts to `Decisions/` folder with status: `superseded`, `archived`, or `active`.

Additional eviction triggers:
- Embedded artifacts (decision is in code, not referenced in 30+ days)
- Resolved errors (learning extracted into a `[D]`)
- Dormant `[S]` seeds (not activated after 90 days)

## Vault File Frontmatter

Every vault file requires frontmatter:
...
Tag domains: `tech/`, `business/`, `product/`, `ai/`, `personal/`
Keep 3–7 tags per document. Project identity goes in `project:` field, not tags.
...
Folders are created on demand — `Decisions/` only appears when the first critical artifact is recorded.

## Storage Rules

| Priority | Where |
|----------|-------|
| critical | Full file in `Decisions/` + inline entry in `_context.md` |
| volatile | Full file in `Decisions/` + inline entry in `_context.md` |
| settled  | Inline entry in `_context.md` only |
| noise    | Discard (default) |

## Retrieval-First Protocol

Before making any decision or generating content from training data, search the vault:

1. Scan project `_context.md` Active Reasoning Artifacts table for matching `[D]` entries
...
- Hardcode vault paths — users configure their vault location during init
